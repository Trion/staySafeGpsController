...
#define SYSTEM_STATUS_LED GPIO_PIN28
#define UPLOAD_DATA_LED GPIO_PIN26
...
int Http_Post(const char * domain, int port,
  const char * path, uint8_t * body, uint16_t bodyLen, char * retBuffer, int bufferLen) {
  uint8_t ip[16];
  bool flag = false;
  uint16_t recvLen = 0;
...
void callback_internet_connection() {
  uint8_t status;
  bool ret = Network_GetAttachStatus( & status);
  if (!ret) {
    Trace(2, "get attach staus fail");
...
void gps_testTask(void * pData) {
  ...
  while (1) {
    if (isGpsOn) {
      ...
      snprintf(requestPath, sizeof(buffer2), "/api/device-control/645c55478a04726ce6575fa1/emergency?lat=%f&lon=%f", latitude, longitude);
      Trace(1, requestPath);

      // *** CHECK INTERNET CONNECTION *** //
      uint8_t status;
      Network_GetActiveStatus( & status);
      Trace(2, "active status:%d", status);
      if (!status) {
      ...
      // *** END CHECK INTERNET CONNECTION STATUS *** //
      GPIO_LEVEL level = 0;
      ...
      if (receiveEve) {
		...
		GPIO_Set(UPLOAD_DATA_LED,GPIO_LEVEL_LOW);
          PM_Restart();
          Trace(1, "no internet connection");
        }
      }
    }
